...
| `apply-patch/` | Adds an `apply_patch` tool for Codex-style `*** Begin Patch` edits. For GPT/OpenAI/Codex models it hides the built-in `edit` tool and enables `apply_patch`; for non-GPT models it hides `apply_patch` and restores `edit` when the extension hid it. Strongly prompts models to call exactly `{ input }`, accepts legacy `{ patch }`/`{ patchText }`/`{ command }`/`{ content }` as recovery aliases, renders Pi-native boxed previews with cached component trees and colored diff bodies, avoids rendering hidden diff lines while collapsed, reuses one parsed file preview for summaries and diffs, shows green/red `+N -M` counts, omits empty `+0 -0` counts for pure moves/no-change patches, shows active targets, and uses bundled platform binaries with partial-failure recovery guidance. | — |
