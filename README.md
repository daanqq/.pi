...
| `skill-dollar.ts` | Adds `$skill-name` skill references: provides autocomplete items and rewrites the first recognized `$skill-name` marker in a prompt to Pi's native `/skill:name` invocation, so the selected skill is loaded even when it is not exposed for model invocation. Additional `$...` markers are left untouched. The `$` autocomplete auto-trigger lives in `ui-editor.ts` because only one extension can own the custom editor. | `$<skill-name>` skill references |
