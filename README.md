...
| `mr-echat.ts` | Automates EChat commit + push + GitLab MR creation through `glab`; when the current `task/EUTP-*` branch already has a commit for the same task, the commit-title picker can reuse the latest matching commit message. | `/mr-echat [commit-title]` |
