...
| `mr-echat.ts` | Automates EChat commit + push with upstream tracking and GitLab MR creation through `glab` without enabling source-branch deletion on merge; generates commit/MR text with `openai-codex/gpt-5.4-mini` at high reasoning. If an MR already exists and a previous task commit title is found, first offers to reuse it without calling the model; generation starts only on request, then the picker can use generated/existing/regenerate/manual title. | `/mr-echat [commit-title]` |
