...
| `footer-ui.ts` | Installs a compact custom footer with two-cell horizontal padding, token/cost/context stats, and selected extension statuses (`codex-quotas`, `deepseek-balance`, `generation-stats`) on the footer right. Context usage is shown as absolute current/window tokens (`44k/272k`) instead of a percentage. Plain footer text uses the theme `text` color; warning/error and extension-provided colors are preserved, including when the footer is truncated. | — |
