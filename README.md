...
| `apply-patch/` | Adds an `apply_patch` tool for Codex-style `*** Begin Patch` edits. For GPT/OpenAI/Codex models it hides the built-in `edit` tool and enables `apply_patch`; for non-GPT models it hides `apply_patch` and restores `edit` when the extension hid it. Strongly prompts models to call exactly `{ input }`, accepts legacy `{ patch }`/`{ patchText }`/`{ command }`/`{ content }` as recovery aliases, renders Pi-native boxed previews with colored diff bodies, collapses very large previews behind the native tool expand keybinding, and uses bundled platform binaries for add/update/delete/move patches with partial-failure recovery guidance. | — |
