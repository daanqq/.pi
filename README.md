...
| `mr-echat.ts` | Automates EChat commit + push with upstream tracking and GitLab MR creation through `glab` without enabling source-branch deletion on merge; generates commit/MR text with `openai-codex/gpt-5.4-mini` at high reasoning. If an MR already exists for the current branch, skips MR-description generation and only prepares the commit title. When the current `task/EUTP-*` branch already has a commit for the same task, the commit-title picker can reuse the latest matching commit message. | `/mr-echat [commit-title]` |
