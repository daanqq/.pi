...
| `gpt-image-context.ts` | Experimental GPT/OpenAI context compressor: in `before_provider_request`, collapses safe old Responses/Chat history into append-only frozen PNG sections so old image bytes can stay prefix-cache stable, keeps recent/unsealed messages as text, preserves exact strings in a plain-text fact sheet, skips secret-like payloads, caches rendered sections under `~/.pi/cache/gpt-image-context`, and logs telemetry to `~/.pi/logs/gpt-image-context`. On by default; set `PI_GPT_IMAGE_CONTEXT=0` or run `/gptimg off` to disable. | `/gptimg [on\|off\|status\|dump-next]` |
