...
| `custom-footer.ts` | Installs a custom footer with cwd, git branch, session name, token/cost/context stats, model/thinking level, and moves selected extension statuses (`codex-quotas`, `deepseek-balance`, `generation-stats`) to the right side. Context usage is shown as absolute current/window tokens (`44k/272k`) instead of a percentage. Plain footer text uses the theme `text` color; the model/thinking label uses the active thinking-level color; warning/error and extension-provided colors are preserved, including when the footer is truncated. | — |
