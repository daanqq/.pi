...
| `skill-dollar.ts` | Adds `$skill-name` shorthand for Pi skill commands: provides autocomplete items and rewrites leading `$skill-name [args]` input to built-in `/skill:name [args]`, preserving Pi's native skill expansion/rendering (`[skill] ... ctrl+o to expand`). The `$` autocomplete auto-trigger lives in `editor-ui.ts` because only one extension can own the custom editor. | `$<skill-name> [args]` shorthand |
