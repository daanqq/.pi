...
| `mr-echat.ts` | Automates EChat commit + push + GitLab MR creation through `glab` without enabling source-branch deletion on merge; when the current `task/EUTP-*` branch already has a commit for the same task, the commit-title picker can reuse the latest matching commit message. | `/mr-echat [commit-title]` |
| `mr-review.ts` | Replaces the old MR review prompt with `/mr-review`: parses GitLab MR URLs, reads source branch from GitLab when available, falls back to commit messages for `EUTP-*`, fetches task details from `urs.esoft.tech` with `PORA_SESSION`, switches the review repo to `master`, fetches MR into `mr-<iid>` under `/home/user/echat/reviews/<repo>`, then sends the current agent a Russian thermo-nuclear review prompt with task-compliance check. | `/mr-review <MR-URL> [MR-URL...]` |
...
| `shake.ts` | Adds an OMP-style `/shake` command: `elide` replaces tool-result messages and giant fenced/XML blocks with short placeholders and saves originals next to the session; `images` removes image blocks. Reloads the session after rewriting context. | `/shake [elide\|images]` |
