...
Interactive Q&A for pi. Models can call the `answer_questions` tool to show an interactive Q&A TUI and receive structured answers. The legacy `/answer` command can still extract questions from the last assistant message and send compiled answers back into the chat.
...
The extension registers `answer_questions`. The model should call it when it needs concrete user input before continuing, especially instead of asking a list of questions in plain text.

For the `grill-me` skill, the tool guidelines tell the model to prefer `answer_questions` even for a single concrete question, usually with `source: "grill-me"` and `mode: "quick"`.
...
```json
{
  ...
  "reason": "Why the answers are needed",
  "source": "grill-me",
  "mode": "quick",
  "questions": [
    {
      "id": "scope",
      "header": "Scope",
      "question": "What should be included in the MVP?",
      "context": "Optional context",
      "options": [
        { "label": "Tool only", "description": "Implement only the model-called tool." }
      ],
      "required": true,
      "multiline": false
    }
...
Modes:
- `quick`: one question, Enter submits immediately, empty Enter is ignored for required questions.
- `full`: multi-question TUI with navigation, templates, drafts, and final confirmation.
...
The Q&A TUI helper is vendored in `qna-tui.ts`, and the tool schema is a plain JSON schema, so auto-discovered local copies can run without `package.json`, `package-lock.json`, `node_modules`, or `@siddr/pi-shared-qna`.
