...
import { Container, getKeybindings, Input, matchesKey, type SelectItem, SelectList, Text } from "@earendil-works/pi-tui";
...
export default function (pi: ExtensionAPI) {
	pi.registerCommand("review", {
		...
		handler: async (args, ctx) => {
			...
			pendingReview = { targets, multiRepo: targets.length > 1 || !(await isGitRepository(pi, ctx.cwd)) };
			awaitingReviewOutput = true;
			pi.sendUserMessage(buildReviewPrompt(targets));
		},
	});
...
async function resolveReviewTargets(
	...
	const selected = await selectRepositories(ctx, repos);
...
async function selectRepositories(
	...
	if (!ctx.hasUI) {
		const selected: ChildRepo[] = [];
		for (const repo of repos) {
			if (await ctx.ui.confirm("Include repository in review?", repo.label)) selected.push(repo);
		}
		return selected;
	}

	return await ctx.ui.custom<ChildRepo[]>((tui, theme, _keybindings, done) => {
		let selectedIndex = 0;
		...
		return {
			render: (width) => {
				const lines = [
					theme.fg("accent", "─".repeat(Math.max(0, Math.min(width, 72)))),
					theme.fg("accent", theme.bold("Select repositories for /review")),
					theme.fg("dim", "↑↓ navigate • space toggle • enter confirm • esc cancel"),
				];
				for (const [index, repo] of repos.entries()) {
					const cursor = index === selectedIndex ? ">" : " ";
					const mark = selected.has(repo.path) ? "●" : "○";
					const line = `${cursor} ${mark} ${repo.label}`;
			...
			handleInput: (data) => {
				if (keybindings.matches(data, "tui.select.cancel")) {
					...
					selectedIndex = Math.min(repos.length - 1, selectedIndex + 1);
				} else if (matchesKey(data, "space")) {
					const repo = repos[selectedIndex];
					if (selected.has(repo.path)) selected.delete(repo.path);
					else selected.add(repo.path);
				} else if (keybindings.matches(data, "tui.select.confirm")) {
					done(repos.filter((repo) => selected.has(repo.path)));
				}
				tui.requestRender();
			},
		};
