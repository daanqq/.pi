...
Клоны в `/home/user/echat/reviews/<repo>` используются только как общий Git object store. Для каждого запуска расширение создаёт отдельный временный workspace:

```text
/home/user/echat/reviews/jobs/review-<id>/
├── tidy-client--mr-2301/
└── tidy-rest--mr-1693/
```

MR загружается в служебный ref `refs/mr-review/<repo>/<iid>`, а актуальная target-ветка — в отдельный ref `refs/mr-review/<repo>/<iid>-base`, после чего commit MR checkout-ится в detached Git worktree. Target берётся из GitLab metadata; если metadata недоступна, используется `master`. Это не даёт устаревшему `origin/master` или `origin/HEAD` расширить diff посторонними коммитами. Агент читает обычными файловыми инструментами именно состояние MR и может безопасно запускать тесты. Разные процессы pi могут одновременно ревьюить даже разные MR одного репозитория; операции `fetch`, `worktree add` и `worktree remove` сериализуются межпроцессным lock на репозиторий.

Workspace живёт до следующего `/review` в этой сессии или до завершения сессии, чтобы после основного ответа можно было задавать уточняющие вопросы. При штатном завершении worktree и временные файлы удаляются автоматически.
...
- untracked-файлы;
- жёсткую границу review scope: находки допустимы только для изменений между переданными merge base и head либо для явно выбранного working tree;
- основную и связанные YouTrack-задачи;
- дополнительный пользовательский контекст.

Готовые shell-команды для diff в запрос больше не добавляются. Skill строит минимальный diff из структурированных `Path`, `Merge base`, `Head` и `Scope`, фиксирует изменённые файлы и строки как границу ревью и может читать остальной код только как контекст.
